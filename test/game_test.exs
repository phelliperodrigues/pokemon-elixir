defmodule ExMon.GameTest do
  ...
  describe "player/0" do
    test "returns info the player" do
      player = Player.build("John Doe", :chute, :soco, :cura)
      computer = Player.build("Robotinik", :chute, :soco, :cura)
      Game.start(computer, player)
      expected_response = Map.get(Game.info(), :player)
      assert expected_response == Game.player()
    end
  end

  describe "turn/0" do
    test "returns info the turn" do
      player = Player.build("John Doe", :chute, :soco, :cura)
      computer = Player.build("Robotinik", :chute, :soco, :cura)
      Game.start(computer, player)
      expected_response = Map.get(Game.info(), :turn)
      assert expected_response == Game.turn()
    end
  end

  describe "fetch_player/1" do
    test "returns info the fetch the player" do
      player = Player.build("John Doe", :chute, :soco, :cura)
      computer = Player.build("Robotinik", :chute, :soco, :cura)
      Game.start(computer, player)
      expected_response = Map.get(Game.info(), :player)
    ...
    test "returns info the fetch the computer" do
      player = Player.build("John Doe", :chute, :soco, :cura)
      computer = Player.build("Robotinik", :chute, :soco, :cura)
      Game.start(computer, player)
      expected_response = Map.get(Game.info(), :computer)
